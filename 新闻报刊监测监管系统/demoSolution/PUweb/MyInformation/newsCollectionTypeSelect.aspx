<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="newsCollectionTypeSelect.aspx.cs" Inherits="PU.web.MyInformation.newsCollectionTypeSelect" %>
...
<head runat="server">
    ...
    <script language="javascript" type="text/javascript">
        ...
        function GetSelectValue() {
            var newsCollectionTypeID = -2;  //保存所选择的报道收藏类别ID,小于0则表示没有选择
            $("#rbl_type").find("input[type=\"radio\"]").each(function(i, j) {
                if (j.checked) {
                    newsCollectionTypeID = j.value;
                    //return;
                }
            })
            if (newsCollectionTypeID < 0) {
                alert("请选择一个类别！");
                return false;
            }

            window.returnValue = newsCollectionTypeID;
            window.close();
        }
