...
function selectOneUser(IDtxtObjID, NametxtObjID) {
    ...
    var url = "../select/selectOneUser.aspx?UserGUID=" + UserGUID + "&UserName=" + UserName;
...
function selectOneDepart(IDtxtObjID, NametxtObjID) {
    ...
    var url = "../select/selectDep.aspx?DepartID=" + DepartID;
    var ret = showModalDialog(url, "Dialog", wz);
...
/*多用户及部门选择
inItIDStr：初始化已选列表的ID串。格式为： 标识ID,user!标识ID,user 多个间以“,”隔开
...
function selectUserOrDepMore(inItIDStr, inItNameStr) {
    ...
    var DataKey = SetExchangeData_RetrunDataKey(DataValue);
    var wz = "dialogWidth:640px; dialogHeight:620px; status:0;center:yes";
    var url = "../select/HZselectIndex.aspx?DataKey=" + DataKey;
